---
title: "VMware Workstation 中扩展 Ubuntu 虚拟机硬盘容量"
...
&emsp;&emsp;这里可以看到我的 Ubuntu 虚拟机文件系统根目录 `/` 挂载到的分区已经 100% 使用了，需要进行扩容了，进入正题；
...
<span style="color:red">注意：这步很重要，不能跳过！</span>

空间不够安装包？删除几个不重要的文件喽~

## 2. 备份数据
&emsp;&emsp;在修改分区之前，建议对重要数据进行备份，以防止意外丢失。首先关闭虚拟机；
...
&emsp;&emsp;这个文件包含了虚拟机的硬盘数据，包括操作系统、应用程序和所有的文件。<br>
&emsp;&emsp;但是，虚拟机的完整备份不仅仅是这个 **`.vmdk` 文件**。一个完整的虚拟机备份还包括以下文件：
...
&emsp;&emsp;为了确保完整备份虚拟机，应该将整个虚拟机文件夹（包含所有文件）复制到备份位置，而不仅仅是 `.vmdk` 文件。假设你的虚拟机文件夹路径是：
```
...
&emsp;&emsp;扩展虚拟磁盘的容量，扩展完成后，VMware Workstaion提示**磁盘已成功扩展。您必须从客户机操作系统内部对磁盘重新进行分区和扩展文件系统。**
...
# ...
```
&emsp;&emsp;其实我点击打开虚拟机，虽然有系统启动的过程，但是之后屏幕就是黑的，进不去，试了下 MobaXterm 之前建立的连接还在，能够用命令进行操作；(这就是需要第 1 步的原因)
...
<br>&emsp;&emsp;虚拟磁盘的大小已经扩展到 60GB，但当前的分区（`sda1`、`sda2`、`sda5`）总共只占用了 20GB 左右，剩余的 40GB 还未被分配到任何分区。
<br>&emsp;&emsp;目标是将未分配的空间添加到 `sda5`（根分区）。由于 `sda5` 位于扩展分区中，需要通过调整分区的大小来完成扩展。
...
+ 重新创建分区
  ...
  1. 创建扩展分区
    ...
    Partition number (3-4, default 3): 3
      First sector (1050624-..., default 1050624): <回车>
      Last sector, +sectors or +size{K,M,G,T} (1050624-..., default ...): <回车>
    ```
    ---
    + 起始扇区：一般按默认值（直接回车）。
  ...
  2. 创建逻辑分区
    ...
    ```
    + 起始扇区：按默认值（直接回车）。
        + 起始扇区必须与之前的 sda5 保持一致。例如，之前的起始扇区为 1052672，则输入此值。
        + 如果起始扇区默认正确，直接回车。
            ```bash
            First sector (xxxx-xxxx, default 1052672): 1052672
            ```
    + 结束扇区：指定分区大小（直接回车使用剩余空间）。
        + 将逻辑分区扩展到扩展分区的最后，直接按回车使用默认值。
          ```bash
          Last sector, +sectors or +size{K,M,G,T} (xxxx-xxxx, default xxxx): <Enter>
          ```

    e. 分区创建完成后，fdisk 会提示已创建
    
    f. 确认逻辑分区创建完成后，输入 p 查看分区表。
  
  3. 检查新分区表
      
    在完成所有分区后，输入 p 打印当前分区表，确认分区配置是否符合你的预期。
  ...
  5. 是否保留原有签名
    ...
    a. 保留原有签名
      ...
      &emsp;&emsp;虽然分区已经扩展，但文件系统仍然保持之前的大小（20GB）。需要扩展文件系统以匹配新分区的大小。
    ...
    d. 验证
      ...
      3. 确保分区数据完好，文件系统已扩展到 59.5GB。
      4. 如果分区能够正常挂载并访问，则操作成功。
